	...
	su -s /bin/sh - peercast -c " \
		...
		# modify source
			...
			if [ ${YT_VER//./} -eq 42 ]; then \
				sed -i -e 's/(_POSIX_C_SOURCE >= 200112L) \&\& ! _GNU_SOURCE/!defined(__GLIBC__) || ( (_POSIX_C_SOURCE >= 200112L) \&\& ! _GNU_SOURCE )/g' /home/peercast/${WORKDIR}/core/unix/strerror.cpp; \
			fi; \
			if [ ${YT_VER//./} -ge 37 ] && [ ${YT_VER//./} -le 42 ]; then \
				sed -i -e '1s/^/#include<limits.h>\n/' /home/peercast/${WORKDIR}/core/unix/usys.cpp; \
				sed -i -e 's/-DADD_BACKTRACE//' /home/peercast/${WORKDIR}/ui/linux/Makefile; \
			fi; \
			## bugfix for alpine pthread_mutex_lock
			sed -i -e 's/inData.lock.lock();/\/\/inData.lock.lock();/' /home/peercast/${WORKDIR}/core/common/pcp.h; \
			sed -i -e 's/outData.lock.lock();/\/\/outData.lock.lock();/' /home/peercast/${WORKDIR}/core/common/pcp.h; \
		# make
		#WORKDIR /root/peercast-yt-${YT_VER}/ui/linux 
		make -C /home/peercast/${WORKDIR}/ui/linux && \
		tar xzf /home/peercast/${WORKDIR}/ui/linux/peercast-yt-linux-${ARCH}.tar.gz -C /home/peercast/ \
	" &&\
...
##	fi; \
	# mv tarball
	mv /home/peercast/${WORKDIR}/ui/linux/peercast-yt-linux-${ARCH}.tar.gz / && \
	# clean up
	apk del .buildtools && \
	#rm -rf ${WORKDIR} && \
